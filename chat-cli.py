...
class ChatClient:
    ...
    def sendgroupmessage(self,group_usernames="xxx,yyy",message="xxx"):
        if (self.tokenid==""):
            return "Error, not authorized"
        string="sendgroup {} {} {} \r\n" . format(self.tokenid,group_usernames,message)
        print(string)
        result = self.sendstring(string)
        if result['status']=='OK':
    ...
    def add_realm(self, realm_id, target_realm_address, target_realm_port):
        if (self.tokenid==""):
            return "Error, not authorized"
        string="addrealm {} {} {} \r\n" . format(realm_id, target_realm_address, target_realm_port)
        result = self.sendstring(string)
        if result['status']=='OK':
    ...
    def get_realm_inbox(self, realm_id):
        ...
        if result['status']=='OK':
            return "Message received from realm {}: {}".format(realm_id, result['message'])
        else:
            return "Error, {}".format(result['message'])
