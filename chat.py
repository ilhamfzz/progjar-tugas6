...
class Chat:
		...
		try:
			...
			if (command=='auth'):
				...
				for w in j[3:]:
					message="{} {}" . format(message,w)
				usernamefrom = self.sessions[sessionid]['username']
				logging.warning("SEND: session {} send message from {} to {}" . format(sessionid, usernamefrom,group_usernames))
				...
				if realm_id in self.realms:
					...
					return error_message("Realm Tidak Ada")
				...
				return error_message("**Protocol Tidak Benar")
		except KeyError:
			return error_message("Informasi tidak ditemukan")
		except IndexError:
			return error_message("--Protocol Tidak Benar")
	def autentikasi_user(self,username,password):
		if (username not in self.users):
			return error_message("Username Tidak Ditemukan")
		if (self.users[username]['password']!= password):
			return error_message("Password Salah")
	...
	def send_message(self,sessionid,username_from,username_dest,message):
		if (sessionid not in self.sessions):
			return error_message("Session Tidak Ditemukan")
		s_fr = self.get_user(username_from)
		s_to = self.get_user(username_dest)
		
		if (s_fr==False or s_to==False):
			return error_message("User Tidak Ditemukan")
	...
	def send_group_message(self, sessionid, username_from, group_usernames, message):
		...
		for username_dest in group_usernames:
			...
			try:
				...
				inqueue_receiver[username_from]=Queue()
				inqueue_receiver[username_from].put(message)
		return ok_message("Message Sent to Group")
	...
	def send_realm_message(self, sessionid, realm_id, username_to, message):
		...
		return ok_message("Message Sent to Realm")
	...
	def send_group_realm_message(self, sessionid, realm_id, group_usernames, message):
		...
		for username_to in group_usernames:
			...
			self.realms[realm_id].put(message)
			self.realms[realm_id].queue.put(message)
		return ok_message("Message Sent to Group in Realm")
